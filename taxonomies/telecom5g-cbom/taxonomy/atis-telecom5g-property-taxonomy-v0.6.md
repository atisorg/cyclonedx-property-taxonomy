# `atis:telecom5g` Namespace Taxonomy (Draft v0.6)
...
| Property | Description | Value type | Cardinality | Property of |
|---|---|---:|---:|---|
| `atis:telecom5g:kdfAlgorithm` | Key derivation function used in 5G key hierarchy (expected: `HMAC-SHA-256`). | string | MAY appear once | `metadata` |
...
### `atis:telecom5g:nfType` controlled values (TR/TS 33.938 scope)
The following values reflect the entity set commonly referenced by TR 33.938 SCAS documents and associated 5G security assessments:

`AMF`, `SMF`, `UPF`, `SEPP`, `AUSF`, `UDM`, `UDR`, `NRF`, `NSSF`, `PCF`, `NEF`, `5G-EIR`, `N3IWF`, `gNB`, `ng-eNB`.

> Note: TS 23.501 distinguishes between 5GC Network Functions and RAN nodes; `gNB`/`ng-eNB` are included here as 33.938-scoped entities for assessment use cases.

| Property | Description | Value type | Cardinality | Property of |
|---|---|---:|---:|---|
| `atis:telecom5g:nfType` | 3GPP entity type in TR/TS 33.938 scope (used for SCAS alignment). Prefer canonical `component.name`; use this property for validation and mixed entity sets (e.g., gNB/ng-eNB). | enum | MAY appear once | `component` |
| `atis:telecom5g:3gppSpecRelease` | 3GPP release against which this NF/entity is certified or assessed (release-specific SCAS context). | enum: `Rel-15`,`Rel-16`,`Rel-17`,`Rel-18`,`Rel-19` | MAY appear once | `component` |
...
- `<Nxx>` MUST be a 3GPP interface identifier (e.g., `N1`, `N2`, `N11`, `N12`, `N32`, `N32-c`, `N32-f`, etc.).
...
## `atis:telecom5g:suci` namespace taxonomy (Subscriber identity protection)

| Property | Description | Value type | Cardinality | Property of |
|---|---|---:|---:|---|
| `atis:telecom5g:suciProtectionScheme` | SUCI protection scheme used for SUPI concealment (applicable to UDM/ARPF). | enum: `null-scheme`,`profile-a`,`profile-b` | MAY appear once | `component` |

## `atis:telecom5g:sbi` namespace taxonomy (Service-Based Interface security)

These properties capture Service-Based Interface (SBI) security controls used between NFs, including mutual authentication expectations.

| Property | Description | Value type | Cardinality | Property of |
|---|---|---:|---:|---|
| `atis:telecom5g:sbi.mutualAuthentication` | Indicates whether mutual authentication is required/implemented for SBI (commonly realized as mTLS). | boolean | MAY appear once | `component` |
| `atis:telecom5g:mTLSRequired` | Alias for `atis:telecom5g:sbi.mutualAuthentication` (kept for compatibility with early drafts). | boolean | MAY appear once | `component` |

## `atis:telecom5g:oauth` namespace taxonomy (NF service authorization)

| Property | Description | Value type | Cardinality | Property of |
|---|---|---:|---:|---|
| `atis:telecom5g:oauthTokenType` | NF access token type used for SBI authorization (expected: `JWT`). | string | MAY appear once | `component` |
| `atis:telecom5g:oauthTokenProtection` | How the token is protected. | enum: `signature`,`mac` | MAY appear once | `component` |
...
## `atis:telecom5g:nas` namespace taxonomy (NAS algorithm selection)

These properties capture the **3GPP short-form NAS algorithm identifiers** used in TS 33.501 (NIAx/NEAx). They are intentionally expressible even for “null” algorithms (NIA0/NEA0) so that policies/validators can detect and flag them when required.

| Property | Description | Value type | Cardinality | Property of |
|---|---|---:|---:|---|
| `atis:telecom5g:nasIntegrityAlgorithm` | NAS integrity algorithm identifier. | enum: `NIA0`,`NIA1`,`NIA2`,`NIA3` | MAY appear multiple times | `component` |
| `atis:telecom5g:nasConfidentialityAlgorithm` | NAS confidentiality algorithm identifier. | enum: `NEA0`,`NEA1`,`NEA2`,`NEA3` | MAY appear multiple times | `component` |
| `atis:telecom5g:upIntegrityAlgorithm` | User plane integrity algorithm identifier (if UP integrity is used/activated). Note: UP integrity is optional to use; if used for user data, NIA0 shall not be used. | enum: `NIA1`,`NIA2`,`NIA3` | MAY appear multiple times | `component` |
...
- **Naming convention (canonical examples):** `N1`, `N2`, `N3`, `N4`, `N6`, `N9`, `N11`, `N12`, `N15`, `N22`, `N32`, `N32-c`, `N32-f`, `Nnrf`, `Npcf`, `Nnef`, `Nbsf`, `Nudm`, `Nudr`, `Nnwdaf`, `Xn`, `F1`, `E1`.
